...
void pathFinder::readPathFromDirMap( const vector2 &end )
{
	...
	} while ( dirMap[curPos.y][curPos.x] != MS_START );
...
void pathFinder::findPath( char (*maze)[CEL], const vector2 &start, const vector2 &end )
{
	// 起点或者终点无效，直接返回
	...
	else
	{
		...
		while ( 1 )
		{
			...
			if( curPos == end )
			{
				dirMap[start.y][start.x] = MS_START;
				readPathFromDirMap( end );
				return;
			}

			// 当前点有效才需要扩展
			if( isPointValid( curPos, maze ) )
			{
				maze[curPos.y][curPos.x] = MS_WALKED;
